...
-Remove all WQA data from this data frame (not QC'd and doesn't include 2023 - will add this data separately)
```{r}
df_SMSCG <- subset(df_SMSCG, station == "GZL" | station == "MAL" | station == "RYC" | station == "TRB" | station == "HON" | station == "SSI" | station == "RVB") #only keeping the CEMP and FRP stations 
...
```
-Since we only want the 2017 data, we can remove all other data from df_CEMP_2017, but first we need to combine the date/time
```{r}
...
unique(df_CEMP_2017$year) #confirm there are no other years except 2017
```

-Add 2023 CEMP data and GZB data from when CEMP managed the station in 2018 and 2019
```{r}
...
df_GZB_CEMP <- read_csv("Data/GZB_CEMP_data.csv") #add older GZB data

df_GZB_CEMP <- df_GZB_CEMP %>% 
  mutate(date = mdy(date)) #format date
         
names(df_GZB_CEMP) %in% names(df_GZL_2023) #confirm they have the same column headers

df_CEMP_2023 <- bind_rows(df_GZL_2023,df_HON_2023,df_MAL_2023,df_RYC_2023,df_RVB_2023,df_SSI_2023, df_GZB_CEMP) #combine all 2023 data and GZB CEMP data in one data frame

df_CEMP_2023 <- df_CEMP_2023 %>% 
  mutate(date_time = paste(date, time)) %>% #combine the date and time columns
  mutate(year = year(date_time)) #add a year column
 
unique(df_CEMP_2023$year) #confirm that we only have years 2018, 2019, and 2023 
```

-Combine all the CEMP data into one data frame
```{r}
...
df_CEMP_comb <- subset(df_CEMP_comb, select = -c(date, time, date_time)) #gets rid of the old date and time columns, since they're not in df_SMSCG

#rename the necessary column headers to match df_SMSCG
...
rm(df_GZL, df_HON, df_MAL, df_RVB, df_RYC, df_SSI, df_GZL_2023, df_HON_2023, df_MAL_2023, df_RVB_2023, df_RYC_2023, df_SSI_2023, df_GZB_CEMP, df_CEMP_2017, df_CEMP_2023) #remove intermediate dfs
```
...
```{r}
df_BDL <- read_csv("Data/BDL_final.csv")
df_BDL <- df_BDL %>%
  add_column(station = "BDL")

df_CSE <- read_csv("Data/CSE_final.csv")
df_CSE <- df_CSE %>%
  add_column(station = "CSE")

df_GOD <- read_csv("Data/GOD_final.csv")
df_GOD <- df_GOD %>%
  add_column(station = "GOD")

df_GZB <- read_csv("Data/GZB_final.csv")
df_GZB <- df_GZB %>%
  add_column(station = "GZB")

df_GZM <- read_csv("Data/GZM_All.csv")
df_GZM <- df_GZM %>%
  add_column(station = "GZM")

df_HUN <- read_csv("Data/HUN_All.csv")
df_HUN <- df_HUN %>%
  add_column(station = "HUN")

df_NSL <- read_csv("Data/NSL_final.csv")
df_NSL <- df_NSL %>%
  add_column(station = "NSL")

df_VOL <- read_csv("Data/VOL_final.csv")
df_VOL <- df_VOL %>%
  add_column(station = "VOL")

df_MSL <- read_csv("Data/MSL_All.csv")
df_MSL <- df_MSL %>%
  add_column(station = "MSL")

names(df_BDL) %in% names(df_CSE) #column names are different

names(df_GZM) %in% names(df_GZB)
```

-Remove unwanted columns and rename columns that don't match (CSE and VOL column names are slightly different than the others)
```{r}
...
df_MSL <- subset(df_MSL, select = c("DATETIME", "DO (mg/L)", "DO - QAQC Flag", "SpCond (µS/cm)", "SpCond - QAQC Flag", "pH", "pH - QAQC Flag", "Temp (°C)", "Temp - QAQC Flag", "Turbidity (NTU)", "Turbidity - QAQC Flag", "Chl a (µg/L)", "Chl - QAQC Flag", "station")) 
...
rm(df_BDL, df_CSE, df_GOD, df_GZB, df_GZM, df_HUN, df_NSL, df_VOL, df_MSL)
```
...
-Determine what TRB data is still missing from df_SMSCG
```{r}
TRB <- df_rawwq %>%
           filter(station == "TRB")
...
```{r}
df_TRB <- read_csv("Data/TRB_data_ESA.csv", locale=locale(encoding="latin1"))  #read in data, wouldn't work without the locale part probably becuase of the extra columns

df_TRB <- df_TRB %>%
  rename("specific_conductance_uScm_value" = "Specific Conductivity (µS/cm)", "temperature_C_value" = "Temperature (C)","dissolved_oxygen_mgL_value" = "DO (mg/L)", "turbidity_NTU_value" = "Turbidity (NTU)", "fluorescence_ugL_value" = "Chlorophyll-a Concentration (µg/L)") %>%
...
-may not need this since we want Oct data to be lumped in with Jun-Sep of the same year (this is only because we care about the Jun-Oct data for each year for this project)
```{r}
#df_comb <- df_comb %>%
...
#remove the FNU and NTU value and code columns
df_comb <- subset(df_comb, select = -c(turbidity_FNU_value, turbidity_FNU_code, turbidity_NTU_value, turbidity_NTU_code))
```
...
#chla_plot_bygroup <- ggplot(df_comb, aes(x = date_time_pst, y = fluorescence_ugL_value, color=managed_by)) +
  #geom_point() + 
  #theme_bw()
#plot(chla_plot_bygroup)
#ggsave("chla_plot_bygroup.png")

#chla_plot_bycode <- ggplot(df_comb, aes(x = date_time_pst, y = fluorescence_ugL_value, color=fluorescence_ugL_code)) +
  #geom_point() + 
  #theme_bw()
...
do_plot_bystation <- ggplot(df_comb, aes(x = date_time_pst, y = dissolved_oxygen_mgL_value, color=station)) +
  geom_point() + 
  theme_bw()
plot(do_plot_bystation)

pH_plot_bystation <- ggplot(df_comb, aes(x = date_time_pst, y = pH_value, color=station)) +
  geom_point() + 
  theme_bw()
plot(pH_plot_bystation)
```
...
range(df_comb$dissolved_oxygen_mgL_value, na.rm = TRUE)
range(df_comb$pH_value, na.rm = TRUE)

```

-Found negative values and extreme outliers for most parameters 
...
#dissolved oxygen 
unique(df_comb$dissolved_oxygen_mgL_code) #has X codes, which indicates bad data
df_comb$dissolved_oxygen_mgL_value <- replace(df_comb$dissolved_oxygen_mgL_value, df_comb$dissolved_oxygen_mgL_code=="X", NA)

#pH
unique(df_comb$pH_code) #has X codes, which indicates bad data
df_comb$pH_value <- replace(df_comb$pH_value, df_comb$pH_code=="X", NA)
...
-Replace DO values below 0 with NA
```{r}
df_comb$dissolved_oxygen_mgL_value <- replace(df_comb$dissolved_oxygen_mgL_value, df_comb$dissolved_oxygen_mgL_value<0, NA)
```

-Replace pH values below 0 and above 14 with NA
```{r}
df_comb$pH_value <- replace(df_comb$pH_value, df_comb$pH_value>14, NA)
df_comb$pH_value <- replace(df_comb$pH_value, df_comb$pH_value<0, NA)
```
...
-Rename df and Remove columns for EDI
```{r}
df_WQ_EDI <- subset(df_comb, select = -c(region, group, water_year_type)) #remove columns we don't need in the EDI data set

df_WQ_EDI <- df_WQ_EDI %>%
  rename("turb_FNU_value" = "turb_comb_value", "turbidity_FNU_code" = "turb_comb_code") #rename turbidity columns to have FNU as the unit
```


-Export data for EDI
```{r}
#use write_csv function to export the cleaned up 2017-2023 WQ data for EDI

write_csv(df_WQ_EDI, "C:/R_coding/SMSCG_MB/Data/SMSCG_wq_data_2017-2023_clean.csv")
```
...
df_comb$salinity <- ec2pss(df_comb$spc_milli, t=25) #add a column that converts spc_milli to salinity using the ec2pss function in the wql library
...
#create the boundaries for the graph shading, we want to highlight everything from Jun-Oct each year
...
-Plot the daily averages by region (these will go into the report) 
```{r}
...
##12 -  Determine Days of Suitable Smelt Habitat
-Remove all months except June-October, since that's the time frame we're looking at for the Summer-Fall Habitat Report
```{r}
...
```{r}
df_summer_fall = mutate(df_summer_fall, turbcut = case_when(turb >= 12 ~1, TRUE~0), salcut = case_when(salinity<=6 ~1, TRUE~0), tempcut = case_when(wt<=22 ~1, TRUE~0)) #adds new columns for water temp, turbidity, and salinity to yield a "1" if they are good delta smelt conditions
...
summarize(good_smelt_habitat_days= sum(good4smelt),
           turbdays = sum(turbcut), tempdays = sum(tempcut), saldays = sum(salcut))
...
df_limitor_region <- df_smelt %>%
  ...
  summarize(good_smelt_habitat= max(good4smelt), turbmax = max(turbcut), tempmax = max(tempcut), salmax = max(salcut))
...
limitor_region_plot <- ggplot(df_limitor_rlong, aes(x = year, y = Days, fill = Metric))+
  ...
  labs(x = "Year") +
  labs(y = "Number of Suitable Habitat Days")
plot(limitor_region_plot)
```
-For the report, put note on plot that the turb data wasn't available for 2017 in the marsh
-Create boxes around the action years
-SMSCG action years = 2023 and 2018
-X2 action years= 2017, 2019, and 2023 
