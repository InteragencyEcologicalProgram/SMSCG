...
#The data set: Catch of Delta Smelt in the Fall Midwater Trawl from 
#Montezuma Slough

#FMWT data is avaialable here: 
...
#load the file:
FMWT_DS = read.csv("countmodels presentation/FMWT_DeltaSmelt.csv")[,-1]
str(FMWT_DS)
...
#if I wanted to investigate factors, such as salinity and temperature, 
#that affect
#smelt catch, I might try a linear model. However, linear models 
#have assumptions of normality and
...
#if you need a review of what these plots mean: 
...
#traditional statistics would now have 
#you try a log-transformation on the data
#except we need to at "1" to each value 
...
#But which distribution should we use? 
#Poisson is the usual first choice for count data.
...
plot(dsglm)
#Not good. We might have overdispersion. 
#Poisson models are genrally good at describing the mean,
...
#Quasipoisson
#A quassipoisson model uses the mean regression function 
#and variance from the Poisson regression,
#but leaves the dispersion parameter unrestricted. 
#(in a Poisson model dispersion = 1)
# Do not write in your report or paper that you used a 
#quasi-Poisson distribution. Just say that
#you did a Poisson GLM, detected overdispersion, 
#and corrected the standard errors
#using a quasi-GLM model where the variance is 
#given by φ × μ, where μ is the
#mean and φ the dispersion parameter. Zuur et al. 2009


dsglm2 = glm(catch~ Station + Secchi + TopEC, 
             family = quasipoisson, data = FMWT_DS2) 
...
#Negative binomial distributions account for overdisperssion, 
#but variance is a quadratic
#function of the mean rather than a linear function of the 
#mean (as in quazipoisson).
#There is lots of discussion as to which is better 
#(e.g. Ver Hoef and Bveng 2007)

dsnb2 = glm.nb(catch~ Station + Secchi + TopEC,  data = FMWT_DS2) 
summary(dsnb2)
plot(dsnb2)
#but even this doesn't look great. 
#The problem is, we have too many zeros, 
#which isn't dealt with well
...
#this is usually because some of the covariates need to be 
#standardized.
#the function "scale" will subtract the mean 
#from the column and divide by its standard
...
#compare the zeroinflated models with a 
#likelihood ratio test to see if we've delt with the 
#overdispersion
...
#Model validation is based on plotting Pearson 
#residuals against the fitted values
#and the Pearson residuals against each explanitory variable. 
#You should not see any pattern (Zuur et al. 2009)
