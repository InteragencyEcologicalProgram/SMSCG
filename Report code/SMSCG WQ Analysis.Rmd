...
-Load in the SMSCG 2018-2023 water quality data from EDI

```{r}
df_SMSCG <- read_csv("https://pasta.lternet.edu/package/data/eml/edi/876/8/aae1f07aefeb88eef5eea4c6f3dc1bf0")

unique(df_SMSCG$year) #confirm that the data includes years 2017-2023
names(df_SMSCG)
...
save(df_averages, file = "Data/df_averages.RData")
```
...
save(df_habitat_region, df_habitat_year, df_smelt,df_limitor_region, file ="data/df_habitat_year.RData")
...
```{r}
habitat_year_plot <- ggplot(df_habitat_year, aes(x = year, y = good_smelt_habitat, fill= water_year_type)) +
...
```{r}
#cleaned up version for BDSC talk
yrtyps = ungroup(df_habitat_region) %>%
  rename(wy=water_year_type ) %>%
  select(year, wy) %>%
  distinct()

limits2024 = data.frame(Metric = "good_smelt_habitat_days", region = c("Bay", "Marsh", "River"),
                        Days = c(118, 120, 109), wy = "Above Normal", year = 2024)


df_limitor_rlong2 = left_join(df_limitor_rlong, yrtyps) %>%
  bind_rows(limits2024) %>%
  mutate(Metric = factor(Metric, levels = c("turbdays", "tempdays" , "saldays","good_smelt_habitat_days" ),
                         labels = c("Turbidity", "Temperature", "Salinity", "Good Habitat")),
  wy = factor(wy, levels = c("critically dry", "dry", "below normal", "Above Normal", "wet"),
              labels= c("C", "D", "BN", "AN", "W")))

limitor_region_plot <- ggplot(df_limitor_rlong2, aes(x = year, y = Days, fill = wy))+
  facet_grid(Metric~region)+
  geom_col(position = "dodge", color = "black")+
  scale_fill_manual(values = c("red", "orange", "yellow", "blue"), name = "water year\ntype")+
  theme_bw()+
  scale_x_continuous(breaks = c(2017,2018, 2019, 2020, 2021, 2022, 2023))+
  theme(legend.position = "top")+
  ylab("Days of Appropriate Smelt Habitat")+
  xlab(NULL)
  
limitor_region_plot

limitor_region_plot2 <- ggplot(filter(df_limitor_rlong2, Metric == "Good Habitat"),
                               aes(x = year, y = Days, fill = wy))+
  facet_grid(.~region)+
  geom_col(position = "dodge", color = "black")+
  scale_fill_manual(values = c("red", "orange", "yellow", "springgreen3", "blue"), name = "water year\ntype")+
  theme_bw()+
...
#just hte number of combined days for BDSC presentation
df_habitat_region = mutate(df_habitat_region, 
                           wy = factor(water_year_type, levels = c("critically dry", "dry", "below normal", "wet")))

habitat_region_plot <- ggplot(df_habitat_region, aes(x = year, y = good_smelt_habitat_days, fill= wy)) +
...
habitat_Bay <- ggplot(filter(df_habitat_region, region == "Bay"), aes(x = year, y = good_smelt_habitat_days, fill= wy)) +
  geom_col(color = "black") +
  theme_bw() +
  facet_wrap(.~region) +
   labs(fill = "Water Year Type") +
...
habitat_Marsh <- ggplot(filter(df_habitat_region, region == "Marsh"), aes(x = year, y = good_smelt_habitat_days, fill= wy)) +
  geom_col(color = "black") +
  theme_bw() +
  facet_wrap(.~region) +
   labs(fill = "Water Year Type") +
