...
#plot for field survey instructions------------------
ggplot()+
...
#ggsave(file = "SMSCG_Plankton_Map_Field_2023.png",type ="cairo-png", scale=2.5, dpi=300)
...
ggplot()+
  ...
  geom_sf(data =region_focal
          ...
          , alpha=0.2)+
  #plot station locations using different shapes and colors for different types of stations
  geom_sf(data= stations, aes(fill= Type, shape= Type), color= "black",  size= 2.5)+
  #add point for SMSCG 
  geom_sf(data= smscg, fill = "black", shape = 23, color= "black",  size= 4.5)+
  scale_shape_manual(
    labels=c(
  ...
  theme_bw()+
  labs(x="Longitude",y="Latitude")
#ggsave(file = "SMSCG_Plankton_Map_Plan_2023.png",type ="cairo-png", scale=2.5, dpi=300)
